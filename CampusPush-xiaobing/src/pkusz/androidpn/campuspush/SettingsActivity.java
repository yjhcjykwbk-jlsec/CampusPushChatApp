...
import pkusz.androidpn.campuspush.util.LogUtil;
...
public class SettingsActivity extends PreferenceActivity {
	...
	private static final String LOGTAG = LogUtil
			.makeLogTag(SettingsActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (notifyPref.isChecked()) {
			notifyPref.setTitle("接收通知");
		} else {
			notifyPref.setTitle("屏蔽通知");
		}
	}
	...
	private PreferenceScreen createPreferenceHierarchy() {
		...
		notifyPref.setTitle("接收通知");
		notifyPref.setSummaryOn("接收所有推送消息");
		notifyPref.setSummaryOff("屏蔽所有推送消息");
		...
		notifyPref
				.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
					public boolean onPreferenceChange(Preference preference,
						...
						if (checked) {
							preference.setTitle("接收通知");
						} else {
							preference.setTitle("屏蔽通知");
						}
						return true;
					}
				});
		...
		vibratePref.setSummary("通知到来震动提示");

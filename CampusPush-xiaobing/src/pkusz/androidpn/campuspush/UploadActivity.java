...
import pkusz.androidpn.campuspush.data.PreferenceData;
...
public class UploadActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 初始化
		PreferenceData.setString(this, "", "上传成功");
	...
	public void startCameraActivity(View view) {
		...
		if (imagePathOk) {
			...
			Toast.makeText(UploadActivity.this, "图像路径错误，摄像头无法启动",
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		// 摄像头照完相
			...
			try {
				...
				// showNextButton = true;
				// 拍照的图片路径
				Log.i("xiaobingo:onActivityResult, camera photo path:",
						mPhotoPath);
			} catch (NullPointerException np) {
				Toast.makeText(UploadActivity.this, "无法保存照片", Toast.LENGTH_LONG)
						.show();
				Log.e("onActivityResult",
						"NullPointerException when saving photo file");
			}
	...
	/*
	 * 当UploadActivity加载完毕后被调用 调用 setPic() 显示最新的图片. Next-button will be shown,
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		if (mPhotoPath != null) {
			...
			setPic();
			/*
			 * if (showNextButton) { Button buttonNext =
			 * (Button)findViewById(R.id.btnNextActivity);
	...
	public void startSubmit(View view) {
		...
		System.out.println("xiaobingo,上传的图片大小(Kbytes):"
				+ uploadImageFile.length() / 1000);
		...
		if (description == null || description.length() == 0 || name == null
				|| name.length() == 0) {
			Toast.makeText(UploadActivity.this, "请输入对图片的描述", Toast.LENGTH_SHORT)
					.show();
			return;
		} else if (description.length() > 100 || name.length() > 100) {
			Toast.makeText(UploadActivity.this, "超出最大输入的100个字符长度",
